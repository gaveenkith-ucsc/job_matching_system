app.controller("editGuidancePhotoController", function ($scope, guidanceProfileService, loginSessionService, $location, $window) {
    ...
    $scope.random = 1;
    $scope.showimg = false;
    guidanceProfileService.getPhoto(loginSessionService.user_index).then(function (obj) {
        console.log(obj);
    ...
    $scope.savePhoto = function () {
        guidanceProfileService.savePhoto($scope.display).then(function (obj) {
            console.log(obj);
            if (obj.data.records[0].status == 'ok') {
                guidanceProfileService.getPhoto(loginSessionService.user_index).then(function (obj) {
                    $scope.random = $scope.random + 1;
                    $scope.cphoto = $scope.cphoto = "App/Backend/" + obj.data.records[0].photo + "?" + Math.random;
